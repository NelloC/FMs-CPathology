...
df['base_name'] = df['id'].astype(str)
df['label_str'] = df['organ'].astype(str).str.strip().str.lower()

unique_classes = sorted(df['label_str'].unique())
class_map = {name: idx for idx, name in enumerate(unique_classes)}

df['label'] = df['label_str'].map(class_map)
base_names = df['base_name'].values
labels = df['label'].values

train_base_names, val_base_names = train_test_split(
    base_names, test_size=0.2, random_state=42, stratify=labels
)

class HubmapTileDataset(Dataset):
    def __init__(self, root_dir, valid_base_names, class_mapping, transform=None):
        ...
        for class_folder in os.listdir(root_dir):
            clean_folder = class_folder.strip().lower()
            if clean_folder not in class_mapping: continue
            
            class_idx = class_mapping[clean_folder]
            class_path = os.path.join(root_dir, class_folder)
            
            for img_name in os.listdir(class_path):
                if not img_name.endswith(('.tiff', '.tif', '.png', '.jpg')): continue
...
train_dataset = HubmapTileDataset(TILES_DIR, train_base_names, class_map, transform=train_transform)
val_dataset = HubmapTileDataset(TILES_DIR, val_base_names, class_map, transform=val_transform)
